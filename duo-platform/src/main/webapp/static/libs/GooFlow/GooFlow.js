...
//初始化区域图的对象
	...
	if(this.$editable){
	  ...
	  this.$workArea.on("click",{inthis:this},function(e){
		...
		while(This.$nodeData["node_"+This.$max]!=null){//add by yonsin
			This.$max++;
		}
		This.addNode("node_"+This.$max,{name:"node_"+This.$max,left:X,top:Y,type:This.$nowType});
		This.$max++;
	  });
	  //划线或改线时用的绑定
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
            if(e.data.inthis.onItemDbClick!=null&&!e.data.inthis.onItemDbClick(this.id,"line",e))return;
			var oldTxt,x,y,from,to;
			var This=e.data.inthis;

			if(!This.$editable)return;//add by yonsin
			if(GooFlow.prototype.useSVG!=""){
	...
	initGroup:function(width,height){
	  ...
	  this.$group.on("dblclick",{inthis:this},function(e){
        ...
        if(e.data.inthis.onItemDbClick!=null&&!e.data.inthis.onItemDbClick(p.id,"area",e))return;
      ...
      var color=["red","yellow","blue","green"];
		...
		while(This.$lineData["area_"+e.data.inthis.$max]!=null){//add by yonsin
			e.data.inthis.$max++;
		}
      e.data.inthis.addArea("area_"+e.data.inthis.$max,
	...
	addNode:function(id,json){
      ...
      }else if(json.type.indexOf(" red")>-1||json.type.indexOf(" green")>-1||json.type.indexOf(" blue")>-1){//add by yonsin
       if(json.type.indexOf(" red")>-1) this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.red,"border-color":GooFlow.prototype.color.red});
       else if(json.type.indexOf(" green")>-1) this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.green,"border-color":GooFlow.prototype.color.green});
       else if(json.type.indexOf(" blue")>-1) this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.blue,"border-color":GooFlow.prototype.color.blue});
		if(GooFlow.prototype.color.mixFont){
			this.$nodeDom[id].find("td:eq(1)").css("color",GooFlow.prototype.color.mixFont);
			this.$nodeDom[id].find(".span").css("color",GooFlow.prototype.color.mixFont);
		}
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart&&!This.$mpTo.data("p")){
                ...
                if(This.$nowType=="dashed"){
                	tmp.dash=true;
					tmp.marked=true;//add by yonsin
				}
				while(This.$lineData["line_"+This.$max]!=null){//add by yonsin
					This.$max++;
				}
				This.addLine("line_"+This.$max,tmp);
				This.$max++;
			}
			else{
				if(lineStart){
		...
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
            ...
            if(e.data.inthis.onItemDbClick!=null&&!e.data.inthis.onItemDbClick(id,"node",e))return;
	...
	parseDatas:function(){
		for(var k1 in this.$nodeData){
			if(this.$nodeData[k1]["left"]&&this.$nodeData[k1]["left"]!=""){
				this.$nodeData[k1]["left"]=parseInt(this.$nodeData[k1]["left"]);
			}
			if(this.$nodeData[k1]["top"]&&this.$nodeData[k1]["top"]!=""){
				 this.$nodeData[k1]["top"]=parseInt(this.$nodeData[k1]["top"]);
			}
        }
		for(var k2 in this.$lineData){
			if(this.$lineData[k2]["left"]&&this.$areaData[k2]["left"]!=""){
				  this.$lineData[k2]["left"]=parseInt(this.$lineData[k2]["left"]);
			}
			if(this.$lineData[k2]["top"]&&this.$areaData[k2]["top"]!=""){
				  this.$lineData[k2]["top"]=parseInt(this.$lineData[k2]["top"]);
			}
		}
        for(var k3 in this.$areaData){
            if(this.$areaData[k3]["left"]&&this.$areaData[k3]["left"]!=""){
                this.$areaData[k3]["left"]=parseInt(this.$areaData[k3]["left"]);
            }
			 if(this.$areaData[k3]["top"]&&this.$areaData[k3]["top"]!=""){
                this.$areaData[k3]["top"]=parseInt(this.$areaData[k3]["top"]);
            }
        }
	},
...
///////////以下为有关画线的方法
	...
	drawLine:function(id,sp,ep,mark,dash,$scale){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			path.setAttribute("stroke-width",mark? 1.4:1.4);//modify by yonsin
	...
	drawPoly:function(id,sp,m1,m2,ep,mark,dash,$scale){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			poly.strokeWeight=mark? "1.2":"1.2";//modify by yonsin
