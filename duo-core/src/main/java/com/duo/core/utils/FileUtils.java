...
public class FileUtils extends org.apache.commons.io.FileUtils {
    ...
    //目录是否存在
    public static void createsDir(String path) {
    ...
    //创建UTF8文件
    public static void createUTF8File(String path, String filename, String data, boolean isdefaultdir) {
        try {
            if (isdefaultdir) {
                path = System.getProperty("user.dir")  + path.substring(1);
            }
            createsDir(path);//创建目录
            File file = new File(path + filename);
            if (file.exists()) {
                clearFile(path + filename);
            }
            //System.out.println("data======="+data);
            Writer ow = new OutputStreamWriter(new FileOutputStream(path + filename), "utf-8");
    ...
    //读取文件返回字符串
    public static StringBuilder readFile2String(String filePath) throws IOException {
        //获取file
    ...
    //写入生成文件
    public void createWriter(String path, String filename) {
        try {
            createsDir(path);//创建目录
